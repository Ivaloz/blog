...
- **运行时间**:由宿主定时任务触发(**当前实际配置为每日 13:00 Asia/Shanghai**),一切窗口计算以**实际运行时刻**为基准;若某日未触发,下次运行时按「自愈补缺」回填缺失日期
- **完整性优先**:先保证 policy、tech、industry、trade、ai、finance 六类都有覆盖,并按各自时间窗口完成采集、分类和去重;不能因为某一类中文来源较少就跳过该类或只更新少数类别。完整覆盖优先于凑数量,低质量条目不得为了填充而加入。
- **密度目标**:每个运行日,除确实无新闻的类别外,各类目标 ≥ 3 条合格新增;达不到时按工作流第 3b 步换更宽入口复扫,**但不放松质量门槛** —— 目标值用于发现「漏采」,不用于凑数。
- **平台语言**:本站是中文平台,资讯条目的 `title`、`summary`、分类说明必须使用简体中文。英文来源可以引用,但必须将标题和摘要翻译成自然、准确的中文;来源名称可保留官方英文名并补充中文说明。
- **中文来源优先**:同等可信度下优先中国官方机构、中文一线媒体和中文专业媒体。英文官方来源只在没有可靠中文一手来源、或事件本身来自海外且需要引用原始公告时使用。中文成品质量优先于来源语言,不能因来源是英文就直接丢弃真实的重要事件。
- **工具依赖**:
...
1. **拉当前状态**:用 `mcp__github__get_file_contents` 读取 6 个分类 JSON 的当前内容(branch=main)
2. **自愈补缺(抓取前必做)**:先从刚读到的 6 个 JSON 里算出**已有数据的日期集合**(取每条 `publishedAt` 的前 10 位),再与「应该有的日期」比对:
   - 「应该有的日期」= 以运行日(Asia/Shanghai)为终点往前回溯 **7 天**,即 `today-6 … today`
   - 这 7 天里**一条数据都没有的日期**就是«缺失日期»(典型成因:定时任务漏跑 / 应用当时没开)
   - 每发现一个缺失日期,就按下文各类的信息源清单**以「补缺模式」单独再采集一遍**:该类时间窗口**放宽到该日期当天 00:00–23:59(+08:00)**
   - **补缺模式只放宽时间窗口,不放宽任何质量规则** —— 真抓 URL、标题校验、`publishedAt` 取真实值、中文成品、来源层级、单日来源占比 ≤60%,全部照旧执行
   - 有缺失日期时,回报里必须**逐日列出**补了哪些类、各几条;某缺失日期某类确实没有合格条目 → 明确写「该日该类无合格新增」,不得静默跳过
   - 缺失日期为 0 时,直接进入正常窗口采集
3. **抓取**:按下文「信息源清单」逐源抓取,各类抓取窗口不同(见后)
3b. **密度兜底复扫(逐类核对,必做)**:完成第 3 步后,逐类核对本窗口内的合格新增条数:
    - 除「该窗口确实无新闻」的情况外,单类目标 **≥ 3 条**
    - 某类 < 3 条时不要就此收工 —— 回到该类的信息源清单,改用**更宽泛的入口**再扫一遍(综合列表页 / 频道首页 / 聚合 RSS),而不是只盯着一两个官方源
    - 复扫遵守全部质量规则(真抓 URL、中文成品、来源层级、单日来源占比 ≤60%)。**复扫是「换更宽的入口」,不是「降低标准」**:宁可最终仍不足 3 条并如实记录,也不得为凑数加入低质量或无法核实来源的条目
    - 复扫后仍 < 3 条 → 在回报里写明「本窗口该类合格新闻偏少,已复扫,最终 N 条」,便于事后排查是不是某个源失效了
4. **清洗**:剔除来源为 `example.com` 的占位条目(我留的种子数据,首次跑可清干净)
5. **归一**:为每条新条目构造 NewsItem(参见字段规约)
6. **路由 + 分类**:按下文「分类与子分类」决定大类与子分类
7. **去重合并**:与对应大类的 existing items 合并(规则见后),保留每类最多 80 条,按 `publishedAt` 倒序
8. **写回**:6 个 JSON 全部刷新 `updatedAt`,即使 items 没新增也允许只更新时间戳(作为心跳)
9. **提交**:`mcp__github__push_files` 单次 commit 推送 6 个文件到 main
10. **回报**:输出本次摘要(见末尾「回报格式」)

### 中文成品检查(提交前必做)

- `title` 和 `summary` 不得保留整句英文、机器翻译腔或中英混杂的半成品。
- 对英文原文:先理解原文,再用简体中文重写标题和摘要,不能只复制英文标题。
- `source` 记录真实来源;英文来源可保留英文域名或机构名,但不代表成品可以使用英文标题。
- 抽样检查本次新增条目;若中文表达不自然、来源不明或无法核实,删除该条目而不是降低质量标准。
- 检查六类是否都完成采集;某类没有可靠新增时明确记录“本窗口无合格新增”,不能默默跳过。
- **机械自检(每次 commit 前必跑)** —— 扫六类 JSON,任何 `title` 不含汉字即违规,必须当场翻译修掉再提交:
  ```bash
  node scripts/check-zh-titles.mjs
  ```
  该脚本逐条扫描 6 类 JSON 的 `title`,列出所有不含汉字的条目并在发现问题时以非 0 退出;CI 侧 `validate-news.yml` 的 `Verify titles contain Chinese` 是同一规则的硬拦截,含英文标题的提交会被直接判失败。
...
```
{
  ...
  "title":       "...",                              // 必填,简体中文成品标题(见「中文成品检查」);忠于原文,可适度精简但不改写
...
- 原文是中文:**主语+谓语+宾语必须一致**;允许删减站点 / 编号 / 时间前缀,**不允许改主体或加猜测**
- 原文是英文:**先用真实标题做对齐校验** —— 实体名(产品名 / 公司名 / 版本号 / 数字)必须与真实标题一致,允许小幅缩写;校验完**必须翻译成简体中文再写入**,英文原文标题**不得**直接进入 `title` 字段
- **不一致 → 用页面真实标题覆盖你的写法;若覆盖后语义不通,丢弃整条**
- ⚠️ **写入的 `title` 必须含汉字。任何一条 `title` 不含汉字 = 翻译未完成,该批不得提交**
